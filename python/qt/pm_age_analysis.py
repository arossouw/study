...
class AppForm(QMainWindow):
	...
	if len(result_text_err) > 1:
	     ...
	     self.table.setColumnCount(result.record().count())
             ...
             index=0
             while (result.next()):
                 self.table.setItem(index,0,QTableWidgetItem(result.value(0).toString()))
                 self.table.setItem(index,1,QTableWidgetItem(result.value(1).toString()))
                 self.table.setItem(index,2,QTableWidgetItem(result.value(2).toString()))
                 self.table.setItem(index,3,QTableWidgetItem(result.value(3).toString()))
                 self.table.setItem(index,4,QTableWidgetItem(result.value(4).toString()))
                 self.table.setItem(index,5,QTableWidgetItem(result.value(5).toString()))
                 self.table.setItem(index,6,QTableWidgetItem(result.value(6).toString()))
                 index = index+1
